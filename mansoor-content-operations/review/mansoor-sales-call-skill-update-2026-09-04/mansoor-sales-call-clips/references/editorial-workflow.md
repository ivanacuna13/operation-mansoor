...
## Mansoor packaging doctrine (selection)

**Primary archive (file-sourced):** `references/sales-call-feedback-2026-09-04/` → `/home/box/mansoor-content-operations/context/mansoor/sales-call-feedback/` (`2026-09-04-mansoor-imessage-upsell-feedback.md` + `.png`). Packaging doctrine — not Frame timestamps. Do not invent beyond the archive.

From the archive (Mansoor iMessage + Ivan follow-up), prefer candidate arcs that teach sales craft over thin proof-only moments when the recording contains them:

- More **“objecting handling”** or **sales presentation** (archive wording).
- Sales value theme: **“how to upsell existing policy holders on more valuable policies.”**
- Lesson shape (archive): already have a policy → do you know how upsell? → find the need → build value.
- Hook when supported by source speech (Ivan): **“I already have a policy.”**

**Soft-defaults:** When Mansoor/Ivan names an upsell/objection/presentation beat, soft-default packaging priority to that beat. Keep **one** skill — do not split until Ivan asks. Never invent dialogue. If the call is only an eligibility/good-news proof with no such beat, cut that truthfully and still apply the concluding resolution headline.
...
5. End on a clean conversational resolution after the peak moment: confirmation, client decision, agreed number, next step, objection handled, or Mansoor's final commitment. If the selected ending sounds like the call continues, cuts off mid-objection, or withholds the result, keep searching forward.
6. After the last kept spoken beat, add a concluding resolution headline/card that states the point of the video in one short sentence. **Soft-default hold: ~7 seconds** (SC05 exemplar). Frame SC04 also names a 5–10 s band; SC06’s approved ~1.867–1.9 s card is an approve-path outlier, not the waiting default. Record exact text and timing in the project spec. Do not invent Frame wording; use reviewer-supplied text when present, otherwise write one short sentence that matches the verified payoff.

Start on speech or immediately before its onset with **Mansoor centered**. The first rendered frame must contain moving source footage and the fully visible opening headline — no frames before that headline. Do not use silent phone setup, dialing, waiting, camera settling, repeated greetings, off-premise conversation, off-center “camera pointed away” opens, a blank lead-in, or a still/freeze-frame intro. The opening headline supplies premise context; Mansoor’s first line supplies authenticity.
...
- Reject flashing, lagging, blank, black, or ~0.5 s leftover frames at joins. A clean cut has continuous picture with no half-second flash of a discarded frame (Frame SC04/05/06 revision_final).
